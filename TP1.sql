...
SELECT p.numero, p.nom, p.numero, TREAT( p.adresse AS adresseWithEmail_type ).adresseEmail AS adresse
FROM Personnes
...
SELECT p.numero, p.nom, p.numero, TREAT( VALUE(p) AS enseignant_type ).grade AS Enseignantgrade
FROM Personnes p
